...
def main():
    ...
    image_file = st.sidebar.file_uploader(
        "Upload Clear Photo Of Your Face.", type=["jpg", "png"]
    )
    ...
    if image_file is not None:
        ...
        with col1:
            cropped_img = st_cropper(
                img, realtime_update=realtime_update, box_color="#e0218a", aspect_ratio=aspect_ratio
            )
            # Resize the image
            cropped_img = cropped_img.resize((img_width, img_height))
            st.image(cropped_img, width=img_width)
            buf = io.BytesIO()
        ...
        with col2:
            if byte_im is not None:
                ...
                make_ken_button = st.sidebar.button(
                    "Make Me Ken", key="make_ken_button", disabled=(byte_im is None)
                )
                if make_ken_button:
